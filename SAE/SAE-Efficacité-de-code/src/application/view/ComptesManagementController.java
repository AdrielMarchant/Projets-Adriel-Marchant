...
public class ComptesManagementController implements Initializable {
	...
	/**
	 * fonction FXML qui permet de modifier un compte (à implémenter -> bouton désactivé)
	 */
	@FXML
	...
	/**
	 * fonction FXML qui permet de supprimer un compte selectionné
	 */
	@FXML
	...
	/**
	 * fonction qui charge une liste de comptes d'un client
	 */
	...
	private void validateComponentState() {
		...
		if (selectedIndice >= 0) {
			...
			this.btnG.setDisable(false);
			if (this.lvComptes.getSelectionModel().getSelectedItem().estCloture.equals("N")) {
				this.btnSupprCompte.setDisable(false);

				this.btnPrelCompte.setDisable(false);
			}
			else {
				this.btnPrelCompte.setDisable(true);
			...
			//this.btnModifierCompte.setDisable(true);
			this.btnSupprCompte.setDisable(true);
			this.btnG.setDisable(true);
			this.btnPrelCompte.setDisable(true);
		}
	}

	/**
	 * Générer relevé en pdf
	 */
	@FXML
	...
	/**
	 * Ouvre l'interface de prélèvements d'un compte
	 */
	@FXML
	private void doPrelCompte() {
		int selectedIndice = this.lvComptes.getSelectionModel().getSelectedIndex();
		if (selectedIndice >= 0) {
			CompteCourant cpt = this.olCompteCourant.get(selectedIndice);
			this.cm.prelevement(cpt);
		}
	}
}
