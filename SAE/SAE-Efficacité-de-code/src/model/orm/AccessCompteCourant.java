...
public class AccessCompteCourant {
	 ...
	 * @throws DatabaseConnexionException
	 */
	public void supprimerCompteCourant(CompteCourant cpt) throws DataAccessException {
		try {
			Connection con = LogToDatabase.getConnexion();
	 ...
	 * @throws ManagementRuleViolation
	 */
	public void creerCompteCourant(CompteCourant cc) throws RowNotFoundOrTooManyRowsException, DataAccessException,
			DatabaseConnexionException, ManagementRuleViolation {
		try {
			if (cc.debitAutorise > 0) {
				cc.debitAutorise = -cc.debitAutorise;
	...
	public void setNextID(CompteCourant compte) {
		try {
			Connection con = LogToDatabase.getConnexion();
			String query = "SELECT seq_compte.nextval FROM dual";
			PreparedStatement pst = con.prepareStatement(query);
			ResultSet rs = pst.executeQuery();
			if (rs.next()) {
				compte.idNumCompte = rs.getInt(1);
			}
			rs.close();
			pst.close();
		} catch (SQLException e) {
