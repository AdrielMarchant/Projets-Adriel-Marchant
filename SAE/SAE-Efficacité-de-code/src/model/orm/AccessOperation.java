...
public class AccessOperation {
	 ...
	 *
	 * @param idNumCompteS compte débité
	 * idNumCompteD compte crédité
	 ...
	 */
	public void doVirement(int idNumCompteS,int idNumCompteR ,double montant  ) throws DataAccessException, DatabaseConnexionException, ManagementRuleViolation {
		try {
			Connection con = LogToDatabase.getConnexion();
			String query = "{call Virer(?, ?, ?, ?)}";
			CallableStatement pst = con.prepareCall(query);
			pst.setInt(1, idNumCompteS);
			pst.setInt(2, idNumCompteR);
			pst.setDouble(3, montant);
			pst.registerOutParameter(4, java.sql.Types.NUMERIC);
			pst.execute();
			int codeRetour = pst.getInt(4);
			if (codeRetour != 0) { // Erreur applicative
				throw new ManagementRuleViolation(Table.Operation, Order.INSERT,
						"Erreur de règle de gestion : virement impossible", null);
			}
	...
	public ArrayList<Operation> getOperationsLike(int idNumCompte, String monthString) throws DataAccessException, DatabaseConnexionException {
		ArrayList<Operation> alResult = new ArrayList<>();

		try {
			Connection con = LogToDatabase.getConnexion();
			String query = "SELECT * FROM Operation where idNumCompte = ? AND to_char(dateOp,'MM/YYYY') LIKE '%" + monthString + "'";
			query += " ORDER BY dateOp";
